...
export class AdminConnectionManager {
  ...
  async fetchWithCache<T>(
    ...
    if (cached) {
      logger.debug('Cache hit', { table, cacheKey }, 'AdminConnectionManager');
      return cached;
    }
    ...
    try {
      ...
      if (error) {
        logger.error('Admin query error', error, { table }, 'AdminConnectionManager');
        return null;
      }
      ...
      logger.error('Admin connection error', error, { table }, 'AdminConnectionManager');
      return null;
    }
  }

  async batchFetch(queries: Array<{
    key: string;
    table: TableName;
    queryBuilder: (query: ReturnType<typeof supabase.from>) => ReturnType<typeof supabase.from>;
    ...
    await Promise.all(
      queries.map(async ({ key, table, queryBuilder }) => {
  ...
  async invalidateCache(pattern?: string): Promise<void> {
    ...
    logger.info('Cache invalidated', { pattern }, 'AdminConnectionManager');
  }
  ...
  async optimizeQueries<T>(
    ...
    const cacheKey = connectionPool.getCacheKey(table, { columns, filters, options });
    const cached = connectionPool.getCached<T[]>(cacheKey);
    ...
    try {
      ...
      if (error) {
        logger.error('Optimized query error', error, { table, columns }, 'AdminConnectionManager');
        return [];
      }
      ...
      logger.error('Optimize queries error', error, { table }, 'AdminConnectionManager');
      return [];
    }
  }
}
